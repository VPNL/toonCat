...
## pRF centers
centers_lateral = read.csv("../data/pRFCenters_KidsTeensAdults_lateralROIs_pSTS_MTG_AdultAvg_new.csv")
...
category = read.csv("../data/catSelectivity_3Runs_diskROIs_10mm.csv")
df.category = read.csv("../data/categorySelectivity.csv")
...
# FWHM X ROI size
df.ROIsize_FWHM <- df.cat_FWHM %>%
  left_join(df.ROIsize_cat_latdors, by = c("sub", "hemi", "group", "ROI", "category", "stream", "age")) %>%
