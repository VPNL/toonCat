function toonCat_roiCheck_mrVista(vistaDir, resultsPath, subjects, hemi, group, toonCatROIs, atlas)
...
tableData = cell(length(subjects), 2 + numel(toonCatROIs) * numel(hemi));
...
for i = 1:length(subjects)
    ...
    for j = 1:numel(toonCatROIs)
        for h = 1:numel(hemi)
            %file_name = sprintf('%s_%s', hemi{h}, toonCatROIs{j});
            file_name = sprintf('%s.%s_%s', hemi{h}, toonCatROIs{j}, atlas);
            %file_path = fullfile(vistaDir, subjects{i}, '3DAnatomy', 'ROIs', [file_name, '.mat']); 
            file_path = fullfile(vistaDir, subjects{i}, '3DAnatomy', 'ROIs', [file_name, '.mat']); 
...
tableName = ['roi_table_vista_', group, '_', atlas, '.csv'];
