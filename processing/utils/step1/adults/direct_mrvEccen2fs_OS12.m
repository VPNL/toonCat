function direct_mrvEccen2fs_OS12(mrSessionDir, fsDir, smooth)
...
rmModel  = fullfile(mrSessionDir, 'Gray/Averages','retModel-cssFit_flippedLRUD-gFit.mat');
...
% rois.
hemis = {'lh','rh'};
% prfDataFile  = fullfile(mrSessionDir, 'Gray/Averages','retModel-cssFit-gFit.mat');
prfDataFile = fullfile(mrSessionDir, 'Gray/Averages','retModel-cssFit_flippedLRUD-fFit.mat');
...
for h = 1:length(hemis)
    ...
    mrVistaMeshMidGray_vertices = mean(cat(3, mrVistaMeshPial.vertices, mrVistaMeshWhite.vertices),3); % send layers seperately here rather than mean
    

    % compute mapping using mrmMapVerticesToGray (mrmMapGrayToVertices):
    vertex2GrayMap = mrmMapVerticesToGray(mrVistaMeshMidGray_vertices, viewGet(hvol,'nodes'), mmPerVox);
    % vertex2GrayMap = mrmMapVerticesToGray(mrVistaMeshMidGray_vertices, viewGet(hvol,'nodes'), mmPerVox, viewGet(hvol,'edges'));

    % [v2gMap sqDist] = mrmMapVerticesToGray(vertices, grayNodes, mmPerVox,
    % [grayEdges]); it seems there are 3 layers (hvol.nodes, row6), and the
    % code takes into account the edges to compute the layers
    ...
    % Loop over every prf parameter
        ...
        % % Get the exponents to convert size map
        % % preallocate space
        % prfFS = nan(size(vertex2GrayMap));
        % 
        % % Select the data that fall within the vertices to map
        % curData = model{1,1}.exponent; 
        % prfFS(verticesToMap) = curData(vertex2GrayMap(verticesToMap));
        % 
        % % store new mapped prf estimates in allData struct
        % allData.n = prfFS;
        ...
        % output the results:
        if smooth
            fname = fullfile(prfFSDir,[curHemi '_prfParams_smooth_layers.mat']);
        else
