function toon_motionCorrect_adults(baseDir, session)
...
if ~exist(fullfile(sessionDir, 'Images', 'Within_Scan_Motion_Est.fig'), 'file')
    hi = motionCompSelScan(hi, 'MotionComp', 1:runCount, ...  %1:runCount
...
if ~exist(fullfile(sessionDir, 'Between_Scan_Motion.txt'), 'file')
    ...
    %targetScans = [1 2 4]; % change for different b/w scan
